...
{
	public static function elp_template_compose($id = 0, $post = 5, $cat = "", $orderby = "", $order = "DESC", $posttype = "Post", $action = "preview")
	{
		...
		if(count($arrRes) > 0)
		{
			...
			foreach ( $postlist as $post ) 
			{
				...
				$post_excerpt = str_ireplace('<p>', '', $post_excerpt);
				$post_excerpt = str_ireplace('</p>', '', $post_excerpt); 
				...
				if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail($post_id)))
				{
					...
					$post_thumbnail_url1 = get_the_post_thumbnail_url($post_id, 'thumbnail');
					//$post_thumbnail_url2 = wp_get_attachment_image_src( get_post_thumbnail_id( $post_id ) );
				}
				...
				if($post_title <> "") {
					$post_title_only = $post_title;
					$post_title = "<a href='".$post_link."' target='_blank' style=''>".$post_title."</a>";
				}
				
				$bodyown = str_replace('###POSTTITLE###', $post_title, $body);
				$bodyown = str_replace('###POSTTITLEONLY###', $post_title_only, $bodyown);
				$bodyown = str_replace('###POSTLINKONLY###', $post_link, $bodyown);
				...
				$bodyown = str_replace('###POSTIMAGELINK###', $post_thumbnail_url1, $bodyown);
